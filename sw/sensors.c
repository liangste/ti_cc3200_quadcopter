...
static struct short3x acclBias3x;
static struct short3x gyroBias3x;
...
// average bias on init - doesn't work with Mahony's algorithm
static void sensors_get_bias() {
    unsigned char reg = 0x3b;
    int i;
    short tmp;
    for (i = 0; i <= 100; i++) {
        I2C_IF_ReadFrom(MPU6050_I2C_ADDR, &reg, 1, &i2cDataBuf[0], 14);
        tmp = (short)(i2cDataBuf[0] << 8 | i2cDataBuf[1]);
        acclBias3x.x += tmp; acclBias3x.x /= 2;
        tmp = (short)(i2cDataBuf[2] << 8 | i2cDataBuf[3]);
        acclBias3x.y += tmp; acclBias3x.y /= 2;
        tmp = (short)(i2cDataBuf[4] << 8 | i2cDataBuf[5]);
        acclBias3x.z += tmp; acclBias3x.z /= 2;
        tmp = (short)(i2cDataBuf[8] << 8 | i2cDataBuf[9]);
        gyroBias3x.x += tmp; gyroBias3x.x /= 2;
        tmp = (short)(i2cDataBuf[10] << 8 | i2cDataBuf[11]);
        gyroBias3x.y += tmp; gyroBias3x.y /= 2;
        tmp = (short)(i2cDataBuf[12] << 8 | i2cDataBuf[13]);
        gyroBias3x.z += tmp; gyroBias3x.z /= 2;
    }
    UART_PRINT("bias register values: accl_x %hd accl_y %hd accl_z %hd gyro_x %hd gyro_y %hd gyro_z %hd\r\n",
        acclBias3x.x, acclBias3x.y, acclBias3x.z,
        gyroBias3x.x, gyroBias3x.y, gyroBias3x.z
    );
}
...
void sensors_update(struct float3x * accl3x, struct float3x * gyro3x) {
    ...
    I2C_IF_ReadFrom(MPU6050_I2C_ADDR, &reg, 1, &i2cDataBuf[0], 14);
    accl3x->x = ((short)(i2cDataBuf[0] << 8 | i2cDataBuf[1]) - acclBias3x.x) / MPU6050_LSB_PER_G;
    accl3x->y = ((short)(i2cDataBuf[2] << 8 | i2cDataBuf[3]) - acclBias3x.y) / MPU6050_LSB_PER_G;
    accl3x->z = ((short)(i2cDataBuf[4] << 8 | i2cDataBuf[5]) - acclBias3x.z) / MPU6050_LSB_PER_G;
    gyro3x->x = ((short)(i2cDataBuf[8] << 8 | i2cDataBuf[9]) - gyroBias3x.x) / MPU6050_LSB_PER_DEG;
    gyro3x->y = ((short)(i2cDataBuf[10] << 8 | i2cDataBuf[11]) - gyroBias3x.y) / MPU6050_LSB_PER_DEG;
    gyro3x->z = ((short)(i2cDataBuf[12] << 8 | i2cDataBuf[13]) - gyroBias3x.z) / MPU6050_LSB_PER_DEG;
